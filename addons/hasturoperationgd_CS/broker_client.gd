class_name HasturCSharpBrokerClient
extends RefCounted
...
func _init(host: String, port: int, executor_type: String = "editor", editor_plugin = null) -> void:
	...
	_executor = ExecutorRouterScript.new(_executor_type, _editor_plugin_ref)
...
func _send_register() -> void:
	...
	for language in ExecutorRouterScript.get_supported_languages():
...
func _handle_execute(data: Dictionary) -> void:
	...
	var language = str(data.get("language", "gdscript"))
	var start_time = Time.get_ticks_msec()
	var result = _executor.execute_code(language, code, {}, _editor_plugin_ref)
